...
Maya's TUI and reports use a warm, literate visual language — light paper base with ink text and vivid accent beacons.

### Fonts

| Role | Family | Weights | Used for |
|------|--------|---------|----------|
| **Display / Headings** | `Fraunces` (Google Fonts) | 300, 400, 500, 600 | Cover title, `h2` section titles, KPI numbers, cost numbers, rec box title. *Italic* for decorative `em` elements. |
| **Body / UI** | `DM Sans` | 300, 400, 500 | Prose, nav links, pros/cons lists, table body, callout text, cover subtitle. |
| **Code / Labels** | `DM Mono` | 300, 400, 500 | Uppercase labels, `h4`, timeline phases, table headers, inline `code`, cost card providers, eyebrows, footer. |

### Colours

| Token | Hex | Used for |
|---|---|---|
| `--ink` | `#1A1A2E` | Primary text, headings, cover background base |
| `--ink2` | `#3D3D5C` | Body prose, table body text |
| `--ink3` | `#7A7A9A` | Muted labels, nav inactive, `h4` mono headers |
| `--ink4` | `#B8B8CC` | Eyebrows, timeline phases, footer |
| `--paper` | `#FAFAF8` | Page background, KPI card bg |
| `--paper2` | `#F2F2EE` | Loop visual bg, table row hover, inline code bg |
| `--paper3` | `#E8E8E2` | Compare bar track |
| `--rule` | `#E0E0D8` | Horizontal rules, dividers, borders |
| `--blue` | `#2B4FFF` | Primary accent — active nav, exec-box border, featured card, KPI number |
| `--blue-lt` | `#EEF1FF` | Exec-box fill, featured card fill, info callout fill |
| `--green` | `#1A7F5A` | Pro card header, success callout, low-severity, green KPI |
| `--green-lt` | `#EBF7F3` | Pro card fill, success callout fill |
| `--amber` | `#B85C00` | Medium-severity, amber KPI |
| `--amber-lt` | `#FFF4E6` | Warn callout fill |
| `--red` | `#CC2B2B` | Con card header, danger callout, high-severity, red KPI |
| `--red-lt` | `#FFF0F0` | Con card fill, danger callout fill |
| `#0D0D1A` | — | Cover background (near-black navy) |
| `#00DFC8` | — | Cover teal accent — canvas nodes, LiteLLM value highlight |
...
|----------|------|------|--------|
| **Android** | Kotlin foreground service + embedded HTTP | `9999` | 16+ modules, functional |
